    ...
    >
      <div className="p-3">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[9px] uppercase tracking-[0.15em] text-[var(--uav-muted)] font-bold">
            {label}
          </span>
        ...
        <div className="text-xl font-black tracking-tighter text-[var(--uav-text)] font-mono">
          {value}
        </div>
      </div>
    </motion.div>
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isAnalyzing &&
          "bg-[var(--uav-primary)]/5 border-[var(--uav-primary)]/20 text-[var(--uav-primary)]",
        !isAnalyzing &&
          hasAnalysis &&
          "bg-[var(--uav-success)]/5 border-[var(--uav-success)]/20 text-[var(--uav-success)]",
        !isAnalyzing &&
          !hasAnalysis &&
    ...
    >
      ...
      />
      {isAnalyzing
        ? "Parsing telemetry & computing metrics..."
        : hasAnalysis
          ? "Analysis complete — data ready"
          : "Upload a .BIN log file to begin"}
    </motion.div>
  );
}
    ...
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-lg font-bold tracking-tight text-[var(--uav-text)]">
              UAV Analysis
            </h1>
            <p className="text-[10px] text-[var(--uav-muted)] uppercase tracking-widest">
              Telemetry & AI Debrief
            </p>
          </div>
        </div>
      </div>
      ...
      <Tabs defaultValue="ai" className="flex-1 flex flex-col min-h-0">
        <TabsList className="bg-[var(--uav-bg-subtle)] border border-white/5 p-1 h-auto shrink-0">
          <TabsTrigger
            value="ai"
            className="text-xs data-[state=active]:bg-[var(--uav-panel)] data-[state=active]:text-[var(--uav-primary)] data-[state=active]:shadow-[0_0_12px_rgba(107,227,255,0.15)] gap-1.5 px-3 py-1.5"
          >
            <MessageSquare className="w-3.5 h-3.5" /> AI Debrief
          </TabsTrigger>
          <TabsTrigger
            value="metrics"
            className="text-xs data-[state=active]:bg-[var(--uav-panel)] data-[state=active]:text-[var(--uav-accent)] data-[state=active]:shadow-[0_0_12px_rgba(244,201,93,0.15)] gap-1.5 px-3 py-1.5"
          >
        ...
        <TabsContent
          forceMount
          value="metrics"
          className="flex-1 min-h-0 mt-3 overflow-y-auto no-scrollbar data-[state=inactive]:hidden"
        >
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              ...
              <MetricCard
                ...
                label="Max Alt"
                value={formatValue(
                  analysis?.metrics.max_altitude_gain_m,
                  1,
                  "m",
                )}
              ...
              <MetricCard
                ...
                label="Max Speed"
                value={formatValue(
                  analysis?.metrics.max_horizontal_speed_mps,
                  2,
                  "m/s",
                )}
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                ...
                analysis?.summary.anomalies.length ? (
                  <>
                  ...
                  <span className="text-[11px] text-[var(--uav-muted)] italic">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1">
                {analysis?.raw_preview.available_messages
                  .slice(0, 15)
                  .map((m: string, i: number) => (
                    <span
                      key={i}
                      className="px-2 py-0.5 rounded-md bg-white/[0.03] border border-white/5 text-[10px] text-[var(--uav-text-secondary)] font-mono"
                    >
                      {m}
                    </span>
                  ))}
              ...
              {analysis && (
                <div className="text-[10px] font-mono text-[var(--uav-muted)] flex items-center gap-4 mt-1.5">
                  <span className="flex items-center gap-1">
                    <Gauge className="w-3 h-3 text-[var(--uav-primary)]" /> GPS:{" "}
                    {formatValue(analysis.sampling.gps_hz, 1, "Hz")}
                  </span>
                  <span className="flex items-center gap-1">
                    <Gauge className="w-3 h-3 text-[var(--uav-accent)]" /> IMU:{" "}
                    {formatValue(analysis.sampling.imu_hz, 1, "Hz")}
                  </span>
                </div>
              )}
            </div>
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen w-full overflow-hidden relative">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        <div
          className="absolute -top-32 right-1/4 w-[700px] h-[700px] rounded-full blur-[180px] animate-glow-pulse"
          style={{
            background:
              "radial-gradient(circle, var(--uav-primary) 0%, transparent 70%)",
            opacity: 0.08,
          }}
        />
        <div
          className="absolute -bottom-32 left-1/6 w-[600px] h-[600px] rounded-full blur-[180px] animate-glow-pulse-gold"
          style={{
            background:
              "radial-gradient(circle, var(--uav-accent) 0%, transparent 70%)",
            opacity: 0.06,
          }}
        />
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] rounded-full blur-[200px]"
          style={{
            background:
              "radial-gradient(circle, var(--uav-primary) 0%, transparent 70%)",
            opacity: 0.03,
      ...
      <header className="lg:hidden flex items-center justify-between p-3 glass-panel z-50 relative">
        ...
        <Sheet>
          ...
          <SheetContent
            side="left"
            className="w-[360px] bg-[var(--uav-bg)] border-r border-white/5 p-4 overflow-y-auto"
